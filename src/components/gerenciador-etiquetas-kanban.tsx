...
import {
  ...
  acaoEditarEtiquetaKanban,
  acaoExcluirEtiquetaKanban,
...
/** Cadastro de etiquetas do Kanban: criar, editar cor e nome, excluir. Registro à parte das etiquetas de nota. */
  ...
  return (
    <div className="flex-1 overflow-y-auto px-8 py-8">
      ...
      <DialogoConfirmar
        ...
        descricao={`Ela sai de ${usos[paraExcluir?.id ?? ""] ?? 0} tarefa(s). As tarefas em si continuam onde estão.`}
        textoBotao="Excluir etiqueta"
        aoFechar={() => definirParaExcluir(null)}
        aoConfirmar={async () => {
          if (!paraExcluir) return null;
          const resposta = await acaoExcluirEtiquetaKanban(paraExcluir.id);
...
function FormularioEdicao({
  ...
  async function salvar() {
    const resposta = await acaoEditarEtiquetaKanban(etiqueta.id, nome, cor, descricao);
    if (!resposta.ok) {
      definirErro(resposta.erro);
      return;
    }
